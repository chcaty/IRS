...
service.interceptors.request.use(config => {
  ...
  // 对指定的白名单API自动放行，一般用于登录和退出 , '/api/menu', '/api/record','/api/dashboard','/api/user',
  let white_list = ['/api/login','/api/menu', '/api/record','/api/dashboard','/api/user']
  for (var a = 0; a < white_list.length; a++) {
    if (url.includes(white_list[a])) {
      return config;
    }
  }
  ...
  for (max = permissions.length; i < max; i++) {
    ...
    console.log(feature.route_match === '');
    ...
    if (feature.route_match === '') {
      ...
      if (url.includes(feature.route_match)) {
        // 正则不匹配， 无权限，继续下一个判断
      } else {
        // 正则匹配，有权限，直接跳出循环
        isPermission = true
