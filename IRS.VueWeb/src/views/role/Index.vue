  ...
  <div id="role">
    <div id="datagrid">
      ...
      <el-table :data="tableData" border style="width: 100%">
        <el-table-column prop="roleId" label="序号" width="70">
        </el-table-column>
        <el-table-column prop="roleName" label="角色" width="180">
        </el-table-column>
        <el-table-column prop="roleDecs" label="描述" width="200">
        </el-table-column>
          ...
          <el-row class="first-row">
            ...
            <el-col :span="12">
              <el-form-item label="描述">
                <el-input v-model="form.roleDecs" placeholder=''></el-input>
              </el-form-item>
            </el-col>
          </el-row>
        </el-form>
      ...
      <el-dialog title="设置角色功能" :visible.sync="isPermission" @close="cancelPermission()" width="30%" @open="setPermissionSelect">
        <el-tree :data="treeData" show-checkbox node-key="permissionId" ref="tree" default-expand-all highlight-current 
          :default-checked-keys="permissions" :props="defaultProps">
        </el-tree>
        <!-- <el-row>
          <el-checkbox-group v-model="CheckList">
            <el-row v-for="li in treeData" :label="li.permissionName" :key="li.permissionId">
                <el-checkbox  border>{{li.permissionName}}</el-checkbox>
            </el-row>
          </el-checkbox-group>
        </el-row> -->
        <span slot="footer" class="dialog-footer">
  ...
  methods: {
       ...
       getPermission().then(res => {
         ...
         getPermissionById(id).then(response => {
           ...
           result.forEach(element => {
             arrayObj.push(element.permissionId)
           });
           this.permissions = arrayObj;
    ...
    updateData() {  // 保存更新数据
      ...
      updateInfo(this.uploadId, this.form).then(response => {
        ...
        let result = response.status_code;
        if (result == 200) {
          let currentId = this.form.roleId;
          let record = 0;
          record = this.tableData.findIndex((val, index) => {
            return val.roleId == currentId;
          });
    ...
    savePermission() {
      ...
      addPermissionInfo(this.uploadId, this.permissions).then(response => {
        //成功执行内容
        let result = response.status_code;
        if (result == 200) {
          this.$message({
